...
class DpQuerySession:
    ...
    def get_count(self, movie_name, rating_threshold, epsilon):
        ...
        if (movie_name, rating_threshold, epsilon) in self._cache:
            return self._cache[(movie_name, rating_threshold, epsilon)]

        true_count = 0
        for entry in self._entries:
            if entry.movie == movie_name and entry.stars >= rating_threshold:
                true_count += 1
        noisy_count = true_count + np.random.laplace(scale=1/epsilon)
        self._cache[(movie_name, rating_threshold, epsilon)] = noisy_count
